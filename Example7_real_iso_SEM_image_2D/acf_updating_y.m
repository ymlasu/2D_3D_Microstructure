function [R_Gaussian_nearestCorr,R_NonGaussianT] = acf_updating_y(N_grid,Vomlume_Frac)


% load('acf_W3.mat','WW_SEM_ACF_Y');
load('acf_sample_autocorrelation.mat');
x = 0:N_grid;
acf_y = acf_y/acf_y(1);
alpha = 1;
dis = alpha*abs(x-x'); 
...
acf_y(length(acf_y)+1:N_grid+1) = acf_y(end);
...
%                 optimal_R_NonGaussian1 = @(R) -1+1/Vomlume_Frac*mvncdf([norminv(Vomlume_Frac),norminv(Vomlume_Frac)],[0, 0],[1 R; R 1])+1/(1-Vomlume_Frac)*mvncdf([-norminv(Vomlume_Frac),-norminv(Vomlume_Frac)],[0, 0],[1 R; R 1]) - R_NonGaussianT(1,i);
